...
class ChromePage:
    ...
    def __init__(self, result: BrowserResult, *, accept_language: str | None = None) -> None:
        self.result = result
    ...
    def __enter__(self) -> CDPClient:
        ...
        try:
            self.process = launch_chrome(self.port, self.profile, stderr=self.stderr, accept_language=self.accept_language)
...
def launch_chrome(
    ...
    *,
    ...
    `accept_language` 固定浏览器语言（UI 和 `Accept-Language`）。断言可见文案的门禁必须传它：
    否则页面语言跟着开发机的 `LANG` 走，同一个门禁在 zh_TW 的机器上看到的是繁体界面。
    """
    args = [
        find_chrome(),
    ...
    if accept_language is not None:
        args[-1:-1] = [f"--lang={accept_language}", f"--accept-lang={accept_language}"]
